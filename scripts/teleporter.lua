...
local function create_teleporter(surface, city)
  if not settings.startup["em_create_teleporters"].value then return end
...
function Teleporter.teleport(player, target_city, source_teleporter, energy_usage)
  ...
  if target and player.teleport(target, surface) then
    ...
    player.print { "em-player.teleported", player.name, target_city.full_name, gps }
    if source_teleporter then
      source_teleporter.energy = source_teleporter.energy - (energy_usage or 0)
    end
    if settings.global.em_drain_energy_on_teleport.value then
      drain_equipment(player)
    end
  else
    local gps = make_gps_tag(target_city.position)
    player.print { "em-player.teleport-failed", player.name, target_city.full_name, gps }
  end
end
...
--  teleporter.minable = false -- for 2.1, this is 'nil' and is set in prototype
  teleporter.energy = 0
  ...
  if not city.is_spawn_city and settings.startup.em_all_teleporters_available.value then
...
function Teleporter.onCityCharted( event )
  ...
  if Utils.getStartupSetting( "em_create_teleporters" ) then
...
function Teleporter.onInit()
  storage.teleporters = {}
  teleporters = storage.teleporters
  world = storage.world
end
...
function Teleporter.onLoad()
  ...
  for _, teleporter in pairs(storage.teleporters) do
